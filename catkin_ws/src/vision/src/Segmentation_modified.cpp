...
cloud_cb (const sensor_msgs::PointCloud2ConstPtr& cloud_msg)
{
  ...
  pass.filter (*cloud_passed);
   ///////////////////////////////Performing Statistical Filtering ////////////////////////////WORKING ON IT!!
  ...
  filtro.setInputCloud (cloud_passed);
  filtro.setMeanK(35);
  filtro.setStddevMulThresh(1.0);
  filtro.filter (*cloud_stplane);

  ////////////////////////////Perform RANSAC filter Table TOP////////////////////////////////
  ...
  seg1.setDistanceThreshold (0.04); //0.02

  seg1.setInputCloud (cloud_stplane);
  ...
  extract.setInputCloud (cloud_stplane);
  extract.setIndices (inliers);
  extract.setNegative (true);//true
  extract.filter (*cloud_ransac);
  ////////////////////////////Perform RANSAC filter Table Body////////////////////////////////WORKING ON IT!

  //Create RANSAC object for the filter method
   pcl::PointCloud<pcl::PointXYZRGB>::Ptr cloud_ransactwo (new pcl::PointCloud<pcl::PointXYZRGB>);
   
   // perform ransac planar filtration to remove table body
    pcl::ModelCoefficients::Ptr coefficientst (new pcl::ModelCoefficients);
    pcl::PointIndices::Ptr inlierst (new pcl::PointIndices);
  // Create the segmentation object
  ...
  // Mandatory
    ...
    seg2.setDistanceThreshold (0.02); //0.02

    seg2.setInputCloud (cloud_ransac);
    seg2.segment (*inlierst, *coefficientst);


  // Create the filtering object
    pcl::ExtractIndices<pcl::PointXYZRGB> extractt;

    extractt.setInputCloud (cloud_ransac);
    extractt.setIndices (inlierst);
    extractt.setNegative (true);//true
    extractt.filter (*cloud_ransactwo);
  ...
  ////////////////////////// Creating the KdTree object for the search method of the extraction/////////////////////////////////
	...
	tree->setInputCloud(cloud_ransactwo);//cloud_ransac
  ...
  // specify euclidean cluster parameters
	...
	ec.setInputCloud(cloud_ransactwo);
