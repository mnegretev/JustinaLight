...
std::vector<int> torque_Nm_to_bits(const std::vector<double>& torque_NM,  std::vector<int>& directions)
{
    ...
    if(torque_NM[0] < 0)
        torque_bits[0] = (directions[i]*torque_NM[i]*MX_BITS_PER_NM*(1/MX_64_K)) + 1024;
    else
        torque_bits[0] = directions[i]*torque_NM[i]*MX_BITS_PER_NM*(1/MX_64_K);

    torque_bits[0] = directions[i]*torque_NM[i]*MX_BITS_PER_NM*(1/MX_106_K);
    for(int i=1; i<torque_NM.size(); i++)
        if(torque_NM[i] == 0)
        torque_bits[i] = 1024;

        if(torque_NM[i] < 0)
            torque_bits[i] = (directions[i]*torque_NM[i]*MX_BITS_PER_NM*(1/MX_64_K)) + 1024;
        else
            torque_bits[i] = directions[i]*torque_NM[i]*MX_BITS_PER_NM*(1/MX_64_K);
