...
def cartesian_traj(tt, pi, pf, vi, vf, ai, af):
    ...
    # Posiciones ocurren en el orden x,y,z,R,P, Orientacion: Row, Pitch, Yaw rango (-180, 180)
    for i in range(6):  # Generar las trayectorias en el espacio cartesiano
    ...
    for i in range(len(p)):    # Para cada punto de la trayectoria
        point = JointTrajectoryPoint()  # Vreamos un objeto que almacena los datos de 1 punto
        point.positions = p6[0,i], p6[1,i],p6[2,i], np.deg2rad(p6[3,i]),np.deg2rad(p6[4,i]), np.deg2rad(p6[5,i])
...
def callback_trajectory(req):  #request es de tipo Pose
    # Pide tiempo de trayectoria y condiciones inicialea de pos,vel y acel 
    t = req.t 
    tt = np.array([0,t])
    # pi = [0.15, 0.7, 0.4, 0.0, 0.0, 0.0]
    pi = [req.p1.positions.x, req.p1.positions.y, req.p1.positions.z, 0.0, 0.0, 0.0]
    #pf = [0.35, 0.4, 0.5, 90, 0.0, 0.0]
    pf = [req.p2.positions.x, req.p2.positions.y, req.p2.positions.z, 90, 0.0, 0.0]
